                        ...
                        entries["custom_fields_recipe"]["rules"].each do |recipe|
                          recipe.each_pair do |k, v|
                            ...
                            if(v == "file")
                             ...
                             # Find if belongs_to assosiation is defined in custom_fiels_recipe 
                             if(v == "belongs_to")
                            	entries.attributes.merge!(recipe["name"] => entries.send(recipe["name"]))
                             end
                             # End Finding if belongs_to assosiation is defined in custom_fiels_recipe 

                             # Find if many to many assosiation is defined in custom_fiels_recipe 
                             if(v == "many_to_many")
                            	entries.attributes.merge!(recipe["name"] => entries.send(recipe["name"]))
                             end
                             # End Finding if many to many assosiation is defined in custom_fiels_recipe 

                             # Find if has many assosiation is defined in custom_fiels_recipe 
                             if(v == "has_many")
                            	entries.attributes.merge!(recipe["name"] => entries.send(recipe["name"]))
                             end
                             # End Finding if many to many assosiation is defined in custom_fiels_recipe 

                             # Find if has one assosiation is defined in custom_fiels_recipe 
                             if(v == "has_one")
                            	entries.attributes.merge!(recipe["name"] => entries.send(recipe["name"]))
                             end
                             # End Finding if has many assosiation is defined in custom_fiels_recipe 
                          end
                        end
